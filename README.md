...
- a `.mender` newer than the running version together with any `.delta`, or two or more newer `.mender` files, is ambiguous: the whole set is refused (`staged-updates-refused`) and nothing is installed. A `.mender` on another channel is never a target and is ignored, however new its timestamp; an unparsable `.delta` name is ignored the same way.
...
The dir is not cleaned by the apply attempt. `CleanupStaleDeltaFiles` reaps a `.delta` whose channel matches the newest local `.mender` token and that is not newer than it, and otherwise relies on a 30-day age backstop; `.delta.tmp` partials are reaped immediately; full `.mender` files are bounded by the base/staged-target keep-set. update-service deliberately never prunes `/data/ota/mdb` or `/data/ota/dbc`, because the `.mender` there is the delta base. A staged set that is never consumed can therefore linger for up to 30 days, and nothing in this retention path installs anything: it is removed without an install or simply sits until the backstop reaps it.
