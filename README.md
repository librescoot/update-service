...
- a `.mender` newer than the running version together with any `.delta`, or two or more newer `.mender` files, is ambiguous: the whole set is refused (`staged-updates-refused`) and nothing is installed. A `.mender` on another channel is never a target and is ignored, however new its timestamp. An unparsable `.delta` name is ignored only for chain candidacy: it never becomes a chain member, but it is still a `.delta` present in the dir, so beside a newer `.mender` it refuses the set like any other delta.
