...
- the default in the gate snapshot: nightly MDB gates, nightly DBC and every other combination does not, an explicit setting wins in both directions, clearing `commit-gate` returns the device to its default, and a channel change moves that default with it
...
1. `fw_printenv bootcount bootlimit upgrade_available` across a plain reboot, an install-and-reboot, and a `mender-update commit`. Measured on an MDB: with `upgrade_available=1` and `bootcount` untouched, the first reboot takes `bootcount` to 1 and the second to 2, past `bootlimit=1`, which reverts the slot and clears `upgrade_available`. Per board, confirm the other half: while a window waits the gate resets `bootcount`, so a reboot during the wait boots the pending slot again, and a boot that cannot run the gate does not reset it.
2. `mender-update rollback` on a board that has just failed a window: confirm it clears the standalone state and that the following reboot lands the previous partition.
3. The probe set against a real boot: which required units are genuinely always active, and how long after `multi-user.target` the other services publish their state. A floor that is too short is a false rollback. Both lists have now been checked against a real boot: the MDB list on a healthy MDB, and the DBC list on a running DBC, where `dbc-dispatcher` is active and `librescoot-version` is inactive after running successfully, which the oneshot acceptance covers. `valkey` is not required on the DBC because the server unit never runs there.
...
6. On a nightly MDB, the `Config: commit gate` line in the startup log with no gate setting and no flag, and then with `--commit-gate=false` passed explicitly: an absent flag must leave the default on, while the explicit flag must turn it off. On a nightly DBC the same line must read off until the gate is enabled there.
7. `performLocalBootUpdate` reboot during a window: its reboot must not revert the pending artifact. It is the shortest reproducer of the bootcount problem, since it reboots minutes after startup without any user action.
