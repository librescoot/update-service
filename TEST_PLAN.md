...
1. `fw_printenv bootcount bootlimit upgrade_available` across a plain reboot, an install-and-reboot, and a `mender-update commit`. The explicit verdict does not depend on it — the rootfs-image module's `ArtifactRollback` always repoints `mender_boot_part` at the pre-install partition, using the partition it recorded at install time when `upgrade_available` is already 0 — but the implicit case does: a board that powers off mid-window comes back on the same uncommitted slot, and only the boot counter would revert that on its own. Either way the marker decides, so confirm which of the two is landing the rollback.
